...
import cn.peon.jfinal.core.MapInjector;
...
public abstract class JfinalxController {
	 ...
	 */
	public JfinalxController setAttr(String name, Object value) {
	 ...
	 */
	public JfinalxController removeAttr(String name) {
	 ...
	 */
	public JfinalxController setAttrs(Map<String, Object> attrMap) {
	 ...
	 */
	public JfinalxController setSessionAttr(String key, Object value) {
	 ...
	 * @param key a String specifying the key of the Object stored in session
	 */
	public JfinalxController removeSessionAttr(String key) {
		HttpSession session = request.getSession(false);
		if (session != null)
	 ...
	 */
	public JfinalxController setCookie(Cookie cookie) {
	 ...
	 */
	public JfinalxController setCookie(String name, String value, int maxAgeInSeconds, String path) {
	 ...
	 */
	public JfinalxController setCookie(String name, String value, int maxAgeInSeconds, String path, String domain) {
	 ...
	 */
	public JfinalxController setCookie(String name, String value, int maxAgeInSeconds) {
	 ...
	 */
	public JfinalxController removeCookie(String name) {
	 ...
	 * Remove Cookie.
	 */
	public JfinalxController removeCookie(String name, String path) {
	 ...
	 * Remove Cookie.
	 */
	public JfinalxController removeCookie(String name, String path, String domain) {
 ...
 /*	by.wc.由于前段无法映射数据库，所以取消直接获取model*/
	/**
	...
	/*	by.wc.add start----------------*/
	/**获取一个request中的对象组装map  按filter中的数据过滤
	 * @param name
	 * @param filter
	 * @return
	 */
	public Map<String,Object> getMap(String name,Map<String, Class<?>> filter){
		return MapInjector.inject(name, request, filter,false);
	}
	 ...
	 * @return
	 */
	public Map<String,Object> getMap( Map<String, Class<?>> filter){
		return getMap("",filter);
	}
	/**获取一个request中的对象组装map  按表名称 获取数据过滤器进行过滤
	 * @param name
	 * @param filter
	 * @return
	 */
	public Map<String,Object> getMap(String name, Class c){
		Object obj=null;
		try {
			obj = TMapUtil._reflect_(c);
			
		} catch (Exception e) {
			e.printStackTrace();
			throw new ActiveRecordException(c+" invoke getBase error!");
		}
		
		if (obj==null) {
			throw new ActiveRecordException(c+" has no filter reg!");
		}
		return getMap(name,(Map<String, Class<?>>) obj);
	}
	
	/**获取一个request中的对象组装map  按表名称 获取数据过滤器进行过滤 同时支持一个String列表进行遮罩
	 * + add
	 * - sub
	 * % filter
	 * @param name
	 ...
	 * @return
	 */
	public Map<String,Object> getMap(String name, Class c,String[] shade){
		Object obj=null;
		try {
			obj = TMapUtil._reflect_(c);
		...
		Map<String, Class<?>> filter = (Map<String, Class<?>>) obj;
		if (shade!=null&shade.length>0) {//进行遮罩
			ArrayList<String> fs = new ArrayList<String>();//排除法需要一次排除 所以特殊处理
			for (String key : shade) {
				if (key!=null&(key.length()>0)) {
					String str = key.substring(1);
					if (key.startsWith("+")) {
						TMapUtil.add(filter, str);
					}
					if (key.startsWith("-")) {
						TMapUtil.sub(filter, str);
					}
					if (key.startsWith("%")) {
						fs.add(key);
						TMapUtil.sub(filter, str);
			...
			if (fs.size()>0) {//一次排除
				String cls[]=new String[fs.size()];
				fs.toArray(cls);
				TMapUtil.filter(filter, cls);
			}
		}
		return getMap(name,filter);
	}
	 ...
	 * @return
	 */
	public Map<String,Object> getMap( Class c) {
		return getMap("",c);
	}
	/**获取一个request中的对象组装map  按表名称 获取数据过滤器进行过滤 同时支持一个String列表进行遮罩
	 * + add
	 ...
	 * @return
	 */
	public Map<String,Object> getMap( Class c,String[] shade) {
		return getMap("",c,shade);
	}

	
	/**从一个map中获取一个指定前缀的map 并赋予新浅醉
	 * @param from
	 * @param fromfix
	 * @param newfix
	 * @return
	 */
	public Map<String,Object> getMapFromMap(Map<String, Object> from,String fromfix,String newfix){
		return TMapUtil.getMapWithFixAndAppendFix(from, fromfix, newfix);
	}
	 ...
	 * @return
	 */
	public Map<String,Object> getMapFromMap(Map<String, Object> from,String fromfix){
		return TMapUtil.getMapWithFix(from, fromfix);
	}
...
/*	
	public static void main(String[] args) {
		...
		Map<String, Object> x = c.getMapFromMap(o, "b.","");
	...
	*/
	 ...
	 * 默认请求当前页的参数
	 */
	public static String __pn_key="pageNumber";
	/**
	 * 默认请求页长的参数
	 */
	public static String __ps_key="pageSize";
	/**
	 * 默认页数值
	 */
	public static int __pn_def=1;
	/**
	 * 默认页长值
	 */
	public static int __ps_def=10;
	/**获取请求页数
	 * @param fix
	 * @return 
	 */
	public int fatchPn(String fix) {
		String para = getPara(fix+__pn_key);
	 ...
	 * @return 
	 */
	public int fatchPs(String fix) {
		String para = getPara(fix+__ps_key);
		if (StrKit.isBlank(para)) {
			return __ps_def;
		}
		return Integer.parseInt(para);
	 ...
	 */
	public JfinalxController setLocaleToCookie(Locale locale) {
		setCookie(I18N_LOCALE, locale.toString(), I18N.getI18nMaxAgeOfCookie());
		return this;
	}
	
	public JfinalxController setLocaleToCookie(Locale locale, int maxAge) {
	 ...
	 */
	public JfinalxController keepPara() {
	 ...
	 */
	public JfinalxController keepPara(String... names) {
	 ...
	 */
	public JfinalxController keepPara(Class type, String name) {
		String[] values = request.getParameterValues(name);
		if (values != null) {
			if (values.length == 1)
	...
	public JfinalxController keepPara(Class type, String... names) {
	...
	public JfinalxController keepModel(Class modelClass, String modelName) {
		Object model = ModelInjector.inject(modelClass, modelName, request, true);
		request.setAttribute(modelName, model);
		return this;
	}
	
	public JfinalxController keepModel(Class modelClass) {
