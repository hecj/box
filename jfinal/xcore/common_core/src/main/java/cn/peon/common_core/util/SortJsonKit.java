...
package cn.peon.common_core.util;
...
public class SortJsonKit {
	...
	public static void setConvertDepth(int convertDepth) {
		...
		SortJsonKit.convertDepth = convertDepth;
	}
	...
	public static void setTimestampPattern(String timestampPattern) {
		...
		SortJsonKit.timestampPattern = timestampPattern;
	}
	
	public static void setDatePattern(String datePattern) {
		if (datePattern == null || "".equals(datePattern.trim()))
			throw new IllegalArgumentException("datePattern can not be blank.");
		SortJsonKit.datePattern = datePattern;
	}
	...
	private static String mapToJson(Map map, int depth) {
        ...
        boolean first = true;
        //edit by wangchao 此处做一个修改 将key进行排序便于定位参数位置 防止因参数传递问题照成的命中率降低问题
		...
		while(iter.hasNext()){
			...
			Object ke = iter.next();
			toKeyValue(String.valueOf(ke),map.get(ke), sb, depth);
		}
        sb.append('}');
		return sb.toString();
	}
