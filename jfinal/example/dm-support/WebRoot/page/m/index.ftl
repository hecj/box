...
function delByIds(ids){
	location="/m/deleteByIds?id="+(ids.join("&id="));
}
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li onclick="location='/m/add'"><span><img src="/static/uimaker/images/t01.png" /></span>添加</li>
    ...
    <table class="tablelist">
        ...
        <tbody>
       	...
       	<#list pg.getList() as m>
        <tr>
       		<td><input class="onecheck" type="checkbox" value="${m.id}" /></td>
       
			<td style="text-align:center;"><@withSub text="${m.id!}" len=8 /></td>
			<td style="text-align:center;"><@withSub text="${m.n!}" len=8 /></td>
			<td style="text-align:center;"><@withSub text="${m.aurl!}" len=8 /></td>
       
       
       		<td><a href="/m/edit/${m.id}" class="tablelink">查看</a>&nbsp;<a href="/m/delete/${m.id}" class="tablelink"> 删除</a></td>
        </tr> 
		</#list>
	...
	<@paginate currentPage=pg.pageNumber totalPage=pg.totalPage totalRow=pg.totalRow actionUrl="/m/" />
