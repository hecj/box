...
function delByIds(ids){
	location="/dbs/deleteByIds?id="+(ids.join("&id="));
}
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li onclick="location='/dbs/add'"><span><img src="/static/uimaker/images/t01.png" /></span>添加</li>
    ...
    <table class="tablelist">
        ...
        <tbody>
       	...
       	<#list pg.getList() as dbs>
        <tr>
       		<td><input class="onecheck" type="checkbox" value="${dbs.id}" /></td>
       
			<td style="text-align:center;"><@withSub text="${dbs.id!}" len=8 /></td>
			<td style="text-align:center;"><@withSub text="${dbs.url!}" len=8 /></td>
			<td style="text-align:center;"><@withSub text="${dbs.u!}" len=8 /></td>
			<td style="text-align:center;"><@withSub text="${dbs.p!}" len=8 /></td>
       
       
       		<td><a href="/dbs/edit/${dbs.id}" class="tablelink">查看</a>&nbsp;<a href="/dbs/delete/${dbs.id}" class="tablelink"> 删除</a></td>
        </tr> 
		</#list>
	...
	<@paginate currentPage=pg.pageNumber totalPage=pg.totalPage totalRow=pg.totalRow actionUrl="/dbs/" />
