...
final class ActionHandler extends Handler {
	...
	public final void handle(String target, HttpServletRequest request, HttpServletResponse response, boolean[] isHandled) {
		if (target.indexOf('.') != -1) {
			if (!(JFinal.me().getAllActionKeys().contains(target))) {
				return ;
			}
		}
		...
		try {
			//开放controller实例化 将控制器交给spring等插件
			JfinalxController controller = Config.getConstants().getControllerInstancer().instance(action);//action.getControllerClass().newInstance();
