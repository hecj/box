...
<script type="text/javascript" src="/static/js/common.js"></script>
  ...
  <link  rel="Stylesheet" href="/static/js/combobox/jquery.combobox.css" />
    <script type="text/javascript" src="/static/js/combobox/jquery-1.3.2-vsdoc2.js"></script>
...
<body>
    ...
    <div class="rightinfo" style="max-height:300px;overflow: scroll;">
 
    
    <span style="color:red">注意：所有查询都做了最多100条的限制,如果当前进行批量的查询 数据可能不准，原则上不允许批量查询!!!</span>
    <table class="tablelist" >
        ...
        <tr>
       		...
       		<td>
       		<input type="radio" name="_query_id" onclick="prex(this,'${q.n!}','${q.id!}')">
       		<a href="javascript:void(0);" onclick="jQuery(this).prev().click();" class="tablelink"> 已执行${q.clc!}次</a>
   ...
   <div id="pre_div" style="display:none;">
	...
	function ex(){
		...
		var inps=jQuery("#pre_con input[name]").get();
		var sx=[];//记录参数顺序
		for(var ii in inps){
			var o=inps[ii];
			if(o.value+""<=0){
		...
		data.id=id;
		data.sx=sx.join(",");
		jQuery.post("/query/ex", data ,function(d){
			$("#cxjg").css("color","red");
